...
const PartnersPage = () => {
  ...
  const partnersSchema = z.object({
    name: z.string().min(4, t("formErrors.name")),
    email: z.string().email(t("formErrors.email")),
    phone: z
      .string()
    ...
    segment: z.string().min(2, t("formErrors.segment")),
    proposal: z.enum(["Divulgação de marca", "Troca de Serviços", "Patrocínio", "Outro"], {
      errorMap: () => ({ message: t("formErrors.proposal") }),
    }),
    publicInfo: z.string().min(2, t("formErrors.publicInfo")),
    budgeting: z.string().min(2, t("formErrors.budgeting")),
    more: z.string().min(10, t("formErrors.more")),
  });
    ...
    <Layout backgroundHeader="bg-secondary-theme" container={false}>
      <main>
        <section className="bg-secondary-theme text-white text-center py-20 px-4 mt-20">
          <h1 className="text-4xl md:text-6xl font-bold mb-4 animate-fade-in-down">{t("title")}</h1>
        ...
        <section id="benefits" className="py-20 bg-surface-card px-4">
          <div className="max-w-6xl mx-auto text-center">
            ...
            <h2 className="text-gray-600 mb-12 max-w-2xl mx-auto">{t("response")}</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {reasonsCard.map((card, index) => (
                <ReasonsCard key={index} icon={card.icon} title={card.title} text={card.text} />
              ))}
            </div>
          </div>
        </section>
          ...
          <div className="max-w-2xl mx-auto">
            ...
            <FormProvider {...methods}>
              <form
                ...
                className="bg-content-inverse p-8 rounded-lg space-y-6"
              >
