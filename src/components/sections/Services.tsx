...
const Services = ({
  ...
  return (
    <div
      className={`${
        bestOption
          ? "scale-105 bg-gradient-to-t from-primary-theme/80  to-content-primary p-1"
          : "p-6"
      }  max-w-[400px] bg-surface-card rounded-2xl h-auto md:h-full group`}
    >
      <div
        className={`${
          bestOption && "bg-[#ffffff] rounded-2xl p-6"
        } relative flex flex-col h-auto md:h-full`}
      >
        {bestOption && (
        ...
        <ul className="mt-4 space-y-3 text-content-secondary flex-grow">
          {characteristics.map((item, index) => (
            ...
            >
              ...
              <p className="flex-1 text-xs md:text-sm bg-gradient-to-l from-secondary-theme via-secondary-theme/80 to-secondary-theme/60  bg-clip-text text-transparent transition-colors duration-700   group-hover:from-purple-500 group-hover:via-pink-400 group-hover:to-purple-700">
                {item}
              </p>
            </motion.li>
          ))}
        ...
        <Link href={paymentLink} rel="noreferrer" aria-label="Link de assinatura" target="_blank">
          <Button
